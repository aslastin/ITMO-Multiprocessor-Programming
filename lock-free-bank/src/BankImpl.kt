...
class BankImpl(override val numberOfAccounts: Int) : Bank {
    ...
    private inner class TransferOp(val fromIndex: Int, val toIndex: Int, val amount: Long) : Op() {
        ...
        private val minIndex = min(fromIndex, toIndex)
        private val maxIndex = max(fromIndex, toIndex)
        ...
        private fun acquireFromTo(): Pair<AcquiredAccount, AcquiredAccount>? {
            val minAccount = acquire(minIndex, this) ?: return null
            val maxAccount = acquire(maxIndex, this) ?: return null
            return if (fromIndex < toIndex) Pair(minAccount, maxAccount) else Pair(maxAccount, minAccount)
        }

        private fun nonReleaseOperation() {
            val (from, to) = acquireFromTo() ?: return
            when {
